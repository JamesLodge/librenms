NOTE: What follows is a very rough list of commands.  This works on a fresh install of FreeBSD 9.1/10.0.
...
NOTE: These instructions assume you are the root user.  If you are not, prepend `sudo` to all shell commands (the ones that aren't at `mysql>` prompts) or temporarily become a user with root privileges with `sudo -s`.
      Almost all software will be built from Ports. Nginx is used over Apache, but either will work. 

Start by creating/updating your port tree 
...
### On the DB Server ###
    ...
    # cd /usr/ports/database/mysql56-server && make install clean
    # cd /usr/ports/net-mgmt/net-snmp&& make install clean
    
     sysrc snmpd_enable="YES"
     sysrc snmpd_conffile="/usr/local/etc/snmpd.conf"
